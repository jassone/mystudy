...
undolog中只会记录一些索引列以及被更新的列的信息。其他列则会依次顺着版本链找，一直找到合适的版本中有改列值为止(如果某列从来没有更新过，则一直找到版本链头部)
...
如果某个版本的数据对当前事务不可见的话，**那就顺着版本链找到下一个版本的数据，继续按照上边的步骤判断可见性**，依此类推，直到版本链中的最后一个版本。如果最后一个版本也不可见的话，那么就意味着该条记录对
